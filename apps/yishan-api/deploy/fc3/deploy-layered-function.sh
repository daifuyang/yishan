...
if [ -z "${YISHAN_API_RUNTIME_LAYER_ARN:-}" ]; then
  if [ ! -f "$LOCK_FILE" ]; then
    ...
    echo "     1. bash $FC_DIR/build-runtime-layer.sh" >&2
    ...
    echo "     3. node $FC_DIR/layer-state.cjs record \\" >&2
    echo "          $ROOT_DIR/package.json $FC_DIR/layer-dependencies.json \\" >&2
  ...
  if ! YISHAN_API_RUNTIME_LAYER_ARN="$(node "$FC_DIR/layer-state.cjs" arn "$ROOT_DIR/package.json" "$FC_DIR/layer-dependencies.json" "$LOCK_FILE" 2>&1)"; then
    echo "❌ $LOCK_FILE 存在但与当前依赖不匹配（fingerprint 已变）" >&2
    echo "   请重新发布 Runtime Layer 并更新 lock：" >&2
    echo "     bash $FC_DIR/build-runtime-layer.sh" >&2
    ...
    echo "     node $FC_DIR/layer-state.cjs record $ROOT_DIR/package.json $FC_DIR/layer-dependencies.json $LOCK_FILE '<new-arn>'" >&2
    exit 1
  fi
