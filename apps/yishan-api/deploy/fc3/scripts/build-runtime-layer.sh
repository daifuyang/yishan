...
node "$FC_DIR/scripts/lib/write-layer-package.cjs" "$SOURCE_PACKAGE" "$LAYER_CONFIG" "$LAYER_NODEJS_DIR/package.json"
node "$FC_DIR/scripts/lib/layer-state.cjs" fingerprint "$SOURCE_PACKAGE" "$LAYER_CONFIG" > "$BUILD_DIR/fingerprint.json"
...
echo "当前 Layer 指纹:"
node "$FC_DIR/scripts/lib/layer-state.cjs" fingerprint "$SOURCE_PACKAGE" "$LAYER_CONFIG"
