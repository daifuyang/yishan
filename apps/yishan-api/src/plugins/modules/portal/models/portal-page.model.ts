...
export class PortalPageModel {
  ...
  static async getPageById (id: number): Promise<PortalPageResp | null> {
    const p = await this.prisma.portalPage.findFirst({
  ...
  static async getPageByPath (path: string) {
    return await this.prisma.portalPage.findFirst({ where: { path, deletedAt: null } })
  }
  ...
  static async updatePage (id: number, req: UpdatePageReq, userId?: number): Promise<PortalPageResp> {
    ...
    const p = await this.prisma.portalPage.update({
      where: { id },
      data,
      include: {
        creator: { select: { username: true } },
  ...
  static async assignTemplate (id: number, templateId: number | null, userId?: number): Promise<PortalPageResp> {
    ...
    const p = await this.prisma.portalPage.findUnique({
      where: { id },
      include: {
        creator: { select: { username: true } },
        updater: { select: { username: true } },
