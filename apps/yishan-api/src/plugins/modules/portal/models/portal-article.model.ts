...
export class PortalArticleModel {
  ...
  private static async getDefaultArticleTemplate () {
    const option = await this.prisma.sysOption.findFirst({
      where: { key: 'defaultArticleTemplateId', deletedAt: null },
      select: { value: true }
    })
    const defaultId = option?.value
  ...
  static async getArticleList (query: ArticleListQuery): Promise<PortalArticleResp[]> {
    ...
    if (keyword) {
      where.OR = [{ title: { contains: keyword } }, { summary: { contains: keyword } }, { content: { contains: keyword } }]
    }
    ...
    const filtered = categoryId ? articles.filter((a) => a.articleCategories.some((c) => c.categoryId === categoryId)) : articles
  ...
  static async getArticleTotal (query: ArticleListQuery): Promise<number> {
    ...
    const where: any = { deletedAt: null }
    if (keyword) where.OR = [{ title: { contains: keyword } }, { summary: { contains: keyword } }, { content: { contains: keyword } }]
    if (status !== undefined) where.status = parseInt(status as string, 10)
    if (startTime) {
      where.createdAt = where.createdAt || {}
      where.createdAt.gte = new Date(startTime)
    ...
    if (categoryId) {
      return await this.prisma.portalArticle.count({ where: { ...where, articleCategories: { some: { categoryId } } } })
    }
  ...
  static async createArticle (req: CreateArticleReq, userId: number): Promise<PortalArticleResp> {
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      if (req.categoryIds && req.categoryIds.length > 0) {
        await tx.portalArticleCategory.createMany({ data: req.categoryIds.map((cid) => ({ articleId: created.id, categoryId: cid })) })
      }
  ...
  static async updateArticle (id: number, req: UpdateArticleReq, userId?: number): Promise<PortalArticleResp> {
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      await tx.portalArticle.update({ where: { id }, data })
      if (req.categoryIds) {
  ...
  static async assignTemplate (id: number, templateId: number | null, userId?: number): Promise<PortalArticleResp> {
    await this.prisma.portalArticle.update({ where: { id }, data: { templateId: templateId ?? null, updaterId: userId } })
    const final = await this.prisma.portalArticle.findUnique({
      where: { id },
      include: {
        articleCategories: { select: { categoryId: true } },
