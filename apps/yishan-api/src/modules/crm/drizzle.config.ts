...
/**
 * crm 模块专属的 drizzle-kit 配置。
 *
 * 每个 module 自带一份该文件，指向自己目录下的 db/schema.ts 与 drizzle/ 输出。
 *
 * 跑迁移（手工）：
 *   npx drizzle-kit --config=apps/yishan-api/src/modules/crm/drizzle.config.ts generate
 *   npx drizzle-kit --config=apps/yishan-api/src/modules/crm/drizzle.config.ts migrate
 *
