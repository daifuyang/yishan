 ...
 * Shop 属性管理页面
 *
 * 功能：属性列表、创建、编辑、删除、属性值子资源管理
 * 表单：DrawerForm（属性）+ DrawerForm（属性值子资源）
 */
...
const Attributes: React.FC = () => {
  ...
  const handleAddValue = async (v: { value: string; sortOrder?: number }) => { if (!valuesDrawer) return false; await postShopV1AttributesIdValues({ id: valuesDrawer.id }, { value: v.value.trim(), sortOrder: v.sortOrder ?? 0, status: 1 }, {}); message.success('已添加'); await loadValues(valuesDrawer.id); return true }

  const handleSave = async (v: Record<string, any>) => {
    if (editing) {
      await patchShopV1AttributesId({ id: editing.id }, { name: v.name, sortOrder: v.sortOrder, status: v.status }, {})
    } else {
      await postShopV1Attributes({ name: v.name, type: 1, sortOrder: v.sortOrder ?? 0, status: v.status ?? 1 }, {})
    }
    ...
    { title: '创建时间', dataIndex: 'createdAt', width: 160, search: false, valueType: 'dateTime' },
    {
      ...
      render: (_, r) => (
        <Space>
          ...
          <a onClick={() => setValuesDrawer(r)}>管理值</a>
          <Popconfirm title="确定删除该属性？" onConfirm={() => handleDelete(r.id)}><a>删除</a></Popconfirm>
        </Space>
      ),
    },
  ]
  ...
  return (
    <PageContainer>
      <ProTable<Attribute>
        ...
        scroll={{ x: 900 }}
        toolBarRender={() => [
          <Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => { setEditing(null); setDrawerOpen(true) }}>新建属性</Button>,
        ]}
      />
      ...
      <DrawerForm
        ...
        open={drawerOpen}
        onOpenChange={(o) => { if (!o) { setEditing(null) }; setDrawerOpen(o) }}
        grid
        drawerProps={{ destroyOnClose: true, maskClosable: false, width: 640 }}
        initialValues={editing ?? { sortOrder: 0, status: 1 }}
        onFinish={handleSave}
      >
        <ProFormText
          name="name"
      ...
      <DrawerForm
        ...
        open={!!valuesDrawer}
        onOpenChange={(o) => !o && setValuesDrawer(null)}
