...
const PageTemplates: React.FC = () => {
  ...
  const columns: ProColumns<API.portalTemplate>[] = [
    ...
    { title: "更新时间", dataIndex: "updatedAt", search: false, valueType: "dateTime" },
    {
      ...
      valueType: "option",
      render: (_, record) => [
        <TemplateForm key="edit" title="编辑模板" type="page" trigger={<a>编辑</a>} initialValues={record} onFinish={handleFormSuccess} />,
        <TemplateSchemaForm key="schema" title="配置结构" type="page" templateId={record.id} trigger={<a>配置结构</a>} onFinish={handleFormSuccess} />,
