...
import { getPositionList, updatePosition, deletePosition } from '@/services/yishan-admin/sysPositions';
...
const PositionList: React.FC = () => {
  ...
  // 获取岗位状态字典
  const positionStatusDict: Array<{ label: string; value: string }> = dictDataMap.position_status || [];

  /**
   * 处理岗位状态变更
   */
  const handleStatusChange = async (id: number, status: string) => {
    const newStatus = status === PositionStatus.ENABLED ? PositionStatus.DISABLED : PositionStatus.ENABLED;
    const res = await updatePosition({ id: String(id) }, { status: newStatus as "0" | "1" });
    if (res.success) {
  ...
  const columns: ProColumns<API.sysPosition>[] = [
    ...
    },
    {
      ...
      valueEnum: positionStatusDict?.length
        ? positionStatusDict.reduce((acc: Record<string, { text: string; status: string }>, item) => {
            acc[item.value] = { text: item.label, status: item.value === "1" ? "Success" : "Default" };
            return acc;
          }, {})
        : {
