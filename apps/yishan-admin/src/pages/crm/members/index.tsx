...
import { addMemberRemark, createMember, getMember, getMembers, updateMember } from '@/services/yishan-admin/crm';
    ...
    <PageContainer>
      <ProTable
        ...
        request={async (params) => {
          const res = await getMembers({ page: params.current, pageSize: params.pageSize, keyword: params.name || params.numberId });
      ...
      <Modal title={editing ? '编辑会员' : '新建会员'} open={open} onOk={async () => {
        ...
        const res = editing?.id ? await updateMember(editing.id, values) : await createMember(values);
        if (res.success) message.success(res.message);
        setOpen(false);
        actionRef.current?.reload();
      }} onCancel={() => setOpen(false)} width={720} destroyOnHidden>
        <Form form={form} layout="vertical">
          <Space wrap align="start">
            <Form.Item name="numberId" label="会员编号"><Input /></Form.Item>
            <Form.Item name="name" label="顾客姓名" rules={[{ required: true }]}><Input /></Form.Item>
          ...
          <Form.Item name="mobile" label="电话"><Input /></Form.Item>
          <Form.Item name="address" label="地址"><Input /></Form.Item>
          <Form.Item name="project" label="项目"><Input /></Form.Item>
        </Form>
      </Modal>
      <Modal title="会员详情" open={detailOpen} onCancel={() => setDetailOpen(false)} footer={null} width={760}>
        <p>编号：{detail?.numberId}，姓名：{detail?.name}，电话：{detail?.mobile}，项目：{detail?.project}</p>
        <Form form={remarkForm} layout="vertical" onFinish={async (values) => {
          ...
          <Form.Item name="content" label="新增备注" rules={[{ required: true }]}><Input.TextArea rows={3} /></Form.Item>
          <Button type="primary" htmlType="submit">添加备注</Button>
        </Form>
        <div style={{ marginTop: 16 }}>
          {(detail?.remarks || []).map((item: any) => (
            <div key={item.id} style={{ borderBottom: '1px solid #eee', padding: '8px 0' }}>{item.user?.username}: {item.content}</div>
          ))}
        </div>
      </Modal>
    </PageContainer>
