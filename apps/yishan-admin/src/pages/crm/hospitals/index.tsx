...
import { App, Button, Cascader, Col, Divider, Drawer, Form, Input, Modal, Popconfirm, Row, Select, Space, Tag } from 'antd';
import dayjs from 'dayjs';
import React, { useEffect, useRef, useState } from 'react';
import { createHospital, deleteHospital, getHospitals, updateHospital, getHospitalAccounts, createHospitalAccount, updateHospitalAccount, deleteHospitalAccount } from '@/services/yishan-admin/crm';
...
const toRegionOptions = (nodes: any[] = []): any[] => nodes.map((node) => ({
  label: node.name,
  value: node.code,
  children: Array.isArray(node.children) && node.children.length > 0 ? toRegionOptions(node.children) : undefined,
}));
...
const HospitalPage: React.FC = () => {
  ...
  const showForm = (record?: any) => {
    setEditing(record);
    const regionCodes = record?.provinceId && record?.cityId && record?.districtId
      ? [record.provinceId, record.cityId, record.districtId]
      : undefined;
    form.setFieldsValue(record ? { ...record, regionCodes } : { hospitalNature: -1, status: 1 });
    setOpen(true);
  };
  ...
  const submit = async () => {
    ...
    const res = editing?.id ? await updateHospital(editing.id, payload) : await createHospital(payload);
  ...
  const submitAccountEdit = async () => {
    ...
    const values = await accountForm.validateFields();
    const res = await updateHospitalAccount(accountHospitalId, accountEditing.userId, values);
    if (!res.success) {
      antMessage.error(res.message);
      return;
    }
  ...
  const columns: ProColumns<any>[] = [
    ...
    { title: '状态', dataIndex: 'status', valueEnum: { 1: { text: '启用', status: 'Success' }, 0: { text: '停用', status: 'Default' } } },
    {
      ...
      width: 240,
      render: (_, record) => [
        <Button key="accounts" type="link" onClick={() => openAccountDrawer(record.id)}>账号管理</Button>,
  ...
  const accountColumns: ProColumns<any>[] = [
    ...
    },
    {
      ...
      search: false,
      render: (_, r: any) => (
        <Tag color={r.status === 1 ? 'green' : 'red'}>{r.status === 1 ? '启用' : '停用'}</Tag>
      ),
    },
    {
      title: '创建时间',
      ...
      search: false,
      width: 170,
      render: (_, r: any) => (r.createdAt ? dayjs(r.createdAt).format('YYYY-MM-DD HH:mm:ss') : '-'),
    },
    {
      title: '更新时间',
      dataIndex: 'updatedAt',
      search: false,
      width: 170,
      render: (_, r: any) => (r.updatedAt ? dayjs(r.updatedAt).format('YYYY-MM-DD HH:mm:ss') : '-'),
    },
    {
      title: '操作',
      valueType: 'option',
      width: 140,
      render: (_, record: any) => [
        <a key="edit" onClick={() => openAccountModal(record)}>编辑</a>,
        <Popconfirm
          key="delete"
          title="确定解除该账号关联?"
          onConfirm={() => handleAccountDelete(record.userId)}
        >
          <a key="delete-link" style={{ color: '#ff4d4f' }}>解除</a>
        </Popconfirm>,
      ],
    },
  ];
    ...
    <PageContainer>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Row gutter={formGutter}>
            <Col {...thirdCol}>
              <Form.Item name="hospitalName" label="医院名称" rules={[{ required: true, message: '请输入医院名称' }]}>
                <Input disabled={Boolean(editing?.id)} placeholder="请输入医院名称" />
              </Form.Item>
            </Col>
            <Col {...thirdCol}>
          ...
          <Divider titlePlacement="left" plain>地址信息</Divider>
          <Row gutter={formGutter}>
            <Col xs={24} md={12}>
            ...
            <Col xs={24} md={12}>
              <Form.Item name="hospitalAddress" label="详细地址"><Input placeholder="请输入详细地址" /></Form.Item>
            </Col>
          </Row>

          <Divider titlePlacement="left" plain>联系方式</Divider>
          <Row gutter={formGutter}>
            <Col {...thirdCol}>
              <Form.Item name="hospitalPhone" label="咨询电话"><Input placeholder="请输入咨询电话" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="hospitalSelling" label="营销电话"><Input placeholder="请输入营销电话" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="hospitalWebsite" label="官网"><Input placeholder="https://" /></Form.Item>
            </Col>
          </Row>
          <Row gutter={formGutter}>
            <Col {...thirdCol}>
              <Form.Item name="doctorName" label="就医联系人"><Input placeholder="联系人姓名" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="doctorPhone" label="就医电话"><Input placeholder="就医联系电话" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="doctorQq" label="就医QQ"><Input placeholder="QQ号" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="receptionName" label="前台联系人"><Input placeholder="联系人姓名" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="receptionPhone" label="前台电话"><Input placeholder="前台联系电话" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="receptionQq" label="前台QQ"><Input placeholder="QQ号" /></Form.Item>
            </Col>
          </Row>

          <Divider titlePlacement="left" plain>交通信息</Divider>
          <Row gutter={formGutter}>
            <Col {...thirdCol}>
              <Form.Item name="busStation" label="公交站"><Input placeholder="公交站点" /></Form.Item>
            </Col>
            <Col xs={24} md={16}>
              <Form.Item name="busAddress" label="公交路线"><Input placeholder="公交路线说明" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="subwayStation" label="地铁站"><Input placeholder="地铁站点" /></Form.Item>
            </Col>
            <Col xs={24} md={16}>
              <Form.Item name="subwayAddress" label="地铁路线"><Input placeholder="地铁路线说明" /></Form.Item>
            </Col>
          </Row>

          <Divider titlePlacement="left" plain>商务政策</Divider>
          <Row gutter={formGutter}>
            <Col {...thirdCol}>
              <Form.Item name="taxiFare" label="出租车费"><Input placeholder="例如: 约35元" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="vipDiscount" label="会员优惠"><Input placeholder="请输入会员优惠" /></Form.Item>
            </Col>
            <Col {...thirdCol}>
              <Form.Item name="returnPoint" label="医院返点"><Input placeholder="例如: 10%" /></Form.Item>
            </Col>
          </Row>

          <Divider titlePlacement="left" plain>补充资料</Divider>
          <Form.Item name="hospitalIntroduction" label="医院简介"><Input.TextArea rows={4} placeholder="请输入医院简介" /></Form.Item>
      ...
      >
        <ProTable
          ...
          toolBarRender={() => [
            <Button key="create" type="primary" icon={<PlusOutlined />} onClick={() => openAccountModal()}>
              新建
            </Button>,
          ]}
        />
