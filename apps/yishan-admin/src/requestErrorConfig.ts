...
import { getAuthorizationHeader, isAccessTokenExpired, isRefreshTokenExpired, clearTokens } from '@/utils/token';
...
export const errorConfig: RequestConfig = {
  ...
  errorConfig: {
    // 错误抛出
    errorThrower: (res) => {
      const { success, code, message, data, errorCode, errorMessage, showType } =
    ...
    errorHandler: async (error: any, opts: any) => {
      ...
      if (error.response?.status === 401) {
        ...
        try {
          ...
          if (refreshResponse.code?.toString().startsWith('200') && refreshResponse.data) {
            // 保存新的token
            const { accessToken, refreshToken: newRefreshToken, accessTokenExpiresIn, refreshTokenExpiresIn } = refreshResponse.data;
            
            // 使用统一的token管理工具保存新token
            const now = Math.floor(Date.now() / 1000);
            localStorage.setItem('accessToken', accessToken);
            localStorage.setItem('refreshToken', newRefreshToken);
            localStorage.setItem('accessTokenExpiry', String(now + (accessTokenExpiresIn || 900)));
            localStorage.setItem('refreshTokenExpiry', String(now + (refreshTokenExpiresIn || 604800)));
