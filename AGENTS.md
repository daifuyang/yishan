...
> 历史 `verify*` / `profile:*` / `release:*` / `migrate:dry-run` / `migrate:verify-apply` / `arch:check` / `docs:check` 命令以及 `scripts/{verify.mjs,arch/,docs/,migrate/}`、`scripts/generate-core-permissions.mjs`、`artifacts/release/`、`artifacts/plugin-catalog.json` 全部已删除，等待按新基线重新设计。
