...
- 装载与 gate：Core 只 AutoLoad `core/routes/**`；插件通过 `api.register`（返回 Fastify 插件的动态 import）声明入口，由 Core 在 `registerPlugin` gate 内挂载到 `api.prefix`，并由 `pluginGate(pluginId)` 统一管控 disabled 状态（详见 ADR-003）。
